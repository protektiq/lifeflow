  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="rounded-2xl bg-white shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="sticky top-0 bg-white border-b-2 border-yellow-200 p-4 sm:p-6 flex items-start justify-between">
            ...
            <div className="space-y-4">
              {conflicts.map((task) => {
                ...
                return (
                  <div
                    key={task.id}
                    className="rounded-xl border-2 border-yellow-300 bg-gradient-to-br from-yellow-50 to-orange-50 p-4 sm:p-5"
                  >
                    <div className="mb-4">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                      <button
                        ...
                        className="flex-1 px-4 py-2 bg-blue-600 text-white font-semibold rounded-full transition-all duration-300 hover:bg-blue-700 hover:scale-105 focus:outline-none focus:ring-4 focus:ring-blue-500/50 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 flex items-center justify-center gap-2"
                      >
                        {isResolving ? (
                          <>
                      ...
                      <button
                        ...
                        disabled={isResolving}
                        className="flex-1 px-4 py-2 bg-green-600 text-white font-semibold rounded-full transition-all duration-300 hover:bg-green-700 hover:scale-105 focus:outline-none focus:ring-4 focus:ring-green-500/50 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 flex items-center justify-center gap-2"
                      >
                        {isResolving ? (
                          <>
                            <RefreshCw className="h-4 w-4 animate-spin" />
        ...
        {/* Footer */}
        <div className="sticky bottom-0 bg-white border-t-2 border-yellow-200 p-4 sm:p-6">
          <button
            onClick={onClose}
